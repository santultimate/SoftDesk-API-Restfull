...
class CommentViewSet(viewsets.ModelViewSet):
    ...
    pagination_class = CustomPagination
    permission_classes = [permissions.IsAuthenticated, IsAuthorOrReadOnly, IsContributor]
    ...
    def get_queryset(self):
        ...
        if project_id and issue_id:
            return Comment.objects.filter(issue_id=issue_id,
                                          ...
                                          issue__project__contributors__user=self.request.user).order_by('created_time')
    ...
    def perform_create(self, serializer):
        ...
        if not project.contributors.objects.filter(project=project, user=self.request.user).exists():
